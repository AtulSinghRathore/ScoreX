...
export function renderAdmin(elements: AppElements, state: AppState): void {
  ...
  elements.adminList.innerHTML = state.admins.map(admin => `<div class="admin-row">
    ...
    <span class="badge ${admin.active ? '' : 'off'}">${admin.active ? escapeHtml(admin.role) : 'DEACTIVATED'}</span>
  ...
  elements.auditLog.innerHTML = state.audit.slice(0, 8).map(entry => `<div class="audit-item">${escapeHtml(entry.action)}<small>${escapeHtml(entry.time)}</small></div>`).join('');
  const liveEvents = state.events.filter(event => event.status === 'live');
  elements.marketControls.innerHTML = liveEvents.length
